...
if __name__ == "__main__":
   ...
   node_processor_sets,node_processor_nodes = qs.match_list(query_list)
   print("node_processor_node",node_processor_nodes)
 
   query_list = []
   query_list = qs.add_match_relationship( query_list,relationship="SITE",label=site_data["site"] )
   query_list = qs.add_match_relationship( query_list,relationship="PROCESSOR",label=site_data["local_node"] )
   query_list = qs.add_match_relationship( query_list,relationship="NODE_PROCESSES" )
   ...
   system_control = System_Control(node_processor_nodes[0],package_nodes[0],generate_handlers)
   cf = CF_Base_Interpreter()
